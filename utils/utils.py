...
def StateVector(terms,n_path):
    ...
    for k, v in terms:
        idx = [k.args[i].indices[0] for i in range(n_path) ]
        amp = complex(v)
        psi[tuple(idx)]=amp
    psi = psi.reshape(-1)
    psi = psi/LA.norm(psi)
...
def action(act_id:int,**kwargs):
    ...
    if act_id == 6:
        return DC0011(kwargs['p1'],kwargs['p2'])
    
    if act_id == 7:
        return DC00(kwargs['p1'],kwargs['p2'])
    
    if act_id == 8:
        return DC11(kwargs['p1'],kwargs['p2'])
...
def print_action(act_id:int,**kwargs):
    ...
    if act_id == 6:
        print('Down Conversion |00>+|11> on path {} and path {}'.format(kwargs['p1'],kwargs['p2']))
    
    if act_id == 7:
        print('Down Conversion |00> on path {} and path {}'.format(kwargs['p1'],kwargs['p2']))

    if act_id == 8:
        print('Down Conversion |11> on path {} and path {}'.format(kwargs['p1'],kwargs['p2']))
...
def generate_random_action(n_path,n_quantization):
    ...
    if act_id == 3 or act_id == 4:
        ...
        theta = int(np.random.default_rng().choice([int(k) for k in range(1,int(n_quantization)+1)],1))
